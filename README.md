...
- **中断续传**：支持停止生成与继续生成，Redis 断点持久化，续传后回答自动合并为一条
...
- **停止生成**：流式输出过程中可随时中断，已生成内容持久化到 Redis（TTL 5 分钟）
- **继续生成**：识别续传指令后从 Redis 加载断点，通过续传提示词引导模型接续生成
- **自动合并**：续传完成后，部分回答与续传回答自动合并为同一条消息，避免回答分裂
- **上下文保护**：续传指令不作为用户消息保存，确保会话标题保持原始提问
- **前端交互**：流式输出时显示"停止生成"按钮，中断后显示"继续生成"按钮
